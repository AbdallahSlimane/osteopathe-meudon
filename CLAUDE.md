...
- **Vanilla JS only**: burger menu (`#burger` / `#navLinks` `.open`), the "Spécialités" dropdown (`.has-drop` / `.drop-toggle` / `.dropdown`) — present on **every** page, since the flat nav cannot hold more than ~6 top-level items above the 1024px burger breakpoint, so new pages go in the dropdown — and on `index.html` a reviews carousel (`#carrTrack` / `#carrDots`, responsive `visibleCount()`). No libraries.
- **Beware the responsive safety rule** `img:not([style*="height"]):not([class]) { height: auto; }` at the top of each `<style>`: its specificity (0,2,1) beats component rules like `.hero-avatar img` (0,1,1). Any `<img>` sized by its container must carry its own inline `height`, as every other photo on the site does.
